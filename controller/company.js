...
module.exports.addApplication = (req, res, next) => {
    ...
    const { fname,
        lname, jobName, skills, eCTC, cCTC,
        email, companyId, resumeUrl, address, currentDesignation, source, noticePeroid
    } = req.body;
    const usr = {
        fname,
        lname, email, companyId, jobName, resumeUrl,
        skills, eCTC, cCTC, address, currentDesignation, source, noticePeroid
    }
...
module.exports.getVisiter = (req, res, next) => {
    jwt.verify(req.token, 'secretkey', (err, authData) => {
        if (err) {
            ...
            User.find({ companyId: id.toString() }).then(applications => {
